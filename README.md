...
4. **Photo strip.** `margin-top: 56px`, `padding-top: 28px`, `border-top` hairline. Header row: kicker "In the shop, on the wall" + a 12px muted note. Then `[data-rsp="plates8"]`: `repeat(8, minmax(0, 1fr))`, `gap: 12px`, each cell `aspect-ratio: 3/4; min-width: 0; overflow: hidden` showing a muted label by default; a slot with a photo has that `<img>` (`object-fit: cover`) absolutely positioned over the label, `opacity: 0` rising to `1` on cell `:hover`/`:focus-within` (shown outright under `hover: none`). Subjects, in order: sample panel (filled), custom colors (filled), trowel detail (filled), crew on site, gilding (filled), tadelakt polished (filled), restoration, finished room (filled). Landscape source photos take a per-slot `position` (`object-position`) to steer the portrait crop. Double-clicking a filled cell opens a ~1600px render of the photo in a modal `<dialog>` lightbox (Esc / backdrop / close button to dismiss).
