...
4. **Photo strip.** `margin-top: 56px`, `padding-top: 28px`, `border-top` hairline. Header row: kicker "In the shop, on the wall" + a 12px muted note. Then `[data-rsp="plates8"]`: `repeat(8, minmax(0, 1fr))`, `gap: 12px`, each cell `aspect-ratio: 3/4; min-width: 0; overflow: hidden` holding a `<image-slot>` at `width/height: 100%`. **In production these are eight `<img>` elements** — the slots are authoring placeholders, currently unfilled. Intended subjects, in order: sample panel, mixing pigment, trowel detail, crew on site, gilding, tadelakt polished, restoration, finished room.
