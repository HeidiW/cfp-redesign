...
4. **Photo strip.** `margin-top: 56px`, `padding-top: 28px`, `border-top` hairline. Header row: kicker "In the shop, on the wall" + a 12px muted note. Then a grid whose column count tracks the number of **filled** slots (`--strip8-cols` / `-md` / `-sm` inline, halving at ≤1080/≤720px), `gap: 12px`, each cell `aspect-ratio: 3/4; min-width: 0; overflow: hidden`. A slot renders **only once its photo is in** — the label sits behind the `<img>` (`object-fit: cover`) at `opacity: 0`, rising to `1` on cell `:hover`/`:focus-within` (shown outright under `hover: none`). Subjects, in order: sample panel, custom colors, trowel detail, **crew on site (awaiting art — hidden)**, gilding, tadelakt polished, **restoration (awaiting art — hidden)**, finished room. Currently 6 of 8 shown. Some source photos take a per-slot `position` (`object-position`) to steer the crop. Each filled cell is a `<button>`: Tab to focus (reveals the photo), Enter/Space or double-click opens a ~1600px render in a modal `<dialog>` lightbox (Esc / backdrop / close button to dismiss; focus returns to the cell).
...
The home-page strip has 6 of 8 photos in (`src/assets/strip/`); "crew on site" and "restoration" are still awaiting art and are hidden until it arrives.
